...
export default function DiaryList({ diaries }) {
  ...
  const {deleteDocument} = useFirestore('diary-app');
  ...
  function formattingTime(seconds) {
    ...
    return `${year}.${String(month).padStart(2, '0')}.${String(date).padStart(2, '0')}.${daysOfWeek[day]}`;
  }
          ...
          return (
            <li key={item.id}>
              <article className={styles["diary-article"]}>
                ...
                <time className={styles["article-time"]} dateTime={formattingTime(item.createTime.seconds).replace(/\./g, '-').slice(0, -4)}>{formattingTime(item.createTime.seconds)}</time>
